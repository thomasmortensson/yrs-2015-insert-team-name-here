...
var currentIcon = L.icon({
    iconUrl: 'img/TBF.png',
    shadowUrl: 'img/emptyShadow.png',

    conSize:     [32, 37], // size of the icon
...
Parse.initialize("qaSySTiZbLnQ4R5jAJu0G7ostvs1cauROnh1RCo9", "98HF9iQCbgagirjU4C5RRbtIZR2YZwkcqPfFErpC");
...
query.find({
    success:function(results) {
      ...
      for (var i = 0; i <= results.length - 1; i++) {
        //alert(results[i].get())
        //alert (results[i].get("location"));
        if( typeof results[i].get("location") != 'undefined') {
          var lat = results[i].get("location").latitude;
          var lon = results[i].get("location").longitude;
          var desc = results[i].get("description");
          var photo = results[i].get("photo");
          var severity = results[i].get("severity");
          var fixed = results[i].get("fixed");
          var buildString = "<div width='200px'>";
          buildString = buildString + "<h4>" + desc +"</h4>";
          buildString = buildString + '<p>Severity: <span class="severity severity'+severity+'"><span>' + severity + "</span></span></p><br/>";
          buildString = buildString + "<img style='display: block; margin-left: auto; margin-right: auto;' src='" + photo.url() + "' width='150px' >";
          buildString = buildString + "</div>";
          if(!fixed) {
            var marker = L.marker([lat, lon], {icon: currentIcon}).addTo(map).bindPopup(buildString);
          }
          else {
            var marker = L.marker([lat, lon], {icon: fixedIcon}).addTo(map).bindPopup(buildString);
          }
          
          var col = document.createElement("td");
          //col.o
          var node = document.createTextNode(desc);
          //node.onclick = "alert('test');"
          var iclass = document.createElement("i");
          iclass.className = "fa fa-chevron-right pull-right"
          
          col.appendChild(node);
          col.appendChild(iclass);
          ...
          row.onclick = function() {
            ...
            markers[this.getAttribute("id")].openPopup(); return false;};
          row.appendChild(col);
      ...
      //document.getElementById('current').appendChild('<td class="Tom here">' + desc + '<i class="fa fa-chevron-right pull-right"></i></td>'); 

      //alert(results[0].get("description"));
      //alert(results[0].get("location").latitude);
    },
...
attributionControl.onAdd = function (map) {
  ...
  div.innerHTML = "Developed by <a href='http://cityradar.co'>Stuff and Things</a> | <a href='#' onclick='$(\"#attributionModal\").modal(\"show\"); return false;'>Attribution</a>";
  return div;
};
...
}).addTo(map);*/ //Removed Layer Control 
